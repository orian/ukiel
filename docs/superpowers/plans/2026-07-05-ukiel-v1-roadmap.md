...
| 5 | ukiel-v1-e2e | `crates/ukiel-e2e` per the testing design doc (`specs/2026-07-05-ukiel-testing-design.md`): docker-compose stack, model-based oracle harness, scenarios S1–S5, then S6–S7 once plan 4 lands | Started early: compose stack, Makefile, `Stack` harness and S0 smoke exist; plan for S1–S7 not written |
| 6 | ukiel-v1-mv | Demo system MV (spec V1 item): change-feed cursor (from plan 4's `worker_cursors`) + SQL transform via DataFusion + append to a target logical table | Not written |
