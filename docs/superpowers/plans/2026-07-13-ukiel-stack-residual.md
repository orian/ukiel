...
> - **POSTSCRIPT (2026-07-13, row 39 executed):** H6 was fixed, and the ≤ 1.15× exit arm this plan could not meet **is now met** — the same-bytes residual is **0.99×** (41 in-scope queries, 27.4 s vs raw's 27.7 s, median 0.98×, ukiel beating raw on 24 of 41). Both systematic overheads are gone: the doubly-evaluated predicate (F3, here) and the `Utf8`-vs-`Utf8View` opt-out (row 39).
>
...
| H5 | The concurrency effect is DataFusion-54-internal (not reachable from provider code). **Named candidate: the sort-pushdown machinery, phases 1–3 of which ship in 54** (epic apache/datafusion#23036; `enable_sort_pushdown = true` by default, `datafusion-common-54.0.0/src/config.rs:1289`). The call chain is live and fed by our declaration: declared ordering → `eq_properties()` → `FileScanConfig::try_pushdown_sort` (`file_scan_config/mod.rs:956`) → `ParquetSource::try_pushdown_sort` (`datafusion-datasource-parquet-54.0.0/src/source.rs:830`) → statistics-based file/row-group reordering + runtime TopK `DynamicFilter`s — **none of it visible in EXPLAIN**, which is exactly the observed signature. Phase-2 file *reordering* is also H1-relevant (composition differences) | the config flag is a clean same-binary lever: repeat the ordering-toggle pairs with `datafusion.optimizer.enable_sort_pushdown = false` on both configs — if the effect vanishes, H5 is pinned; `clickbench plan`'s group dump then shows the reordering directly | defer to row 38 with the path named (Phase 4 / PR #22450 targets DF 55); if pinned, also evaluate actively implementing catalog-aware ordering for the Exact/Inexact TopK paths as its own row (audit note §already-in-54) |
