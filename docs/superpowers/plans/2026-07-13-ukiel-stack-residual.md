...
| H5 | The concurrency effect is DataFusion-54-internal (not reachable from provider code). **Named candidate: the sort-pushdown Phase-1 machinery** — PR #19064 shipped in DF 52, so the `PushdownSort` rule runs by default in 54 (`enable_sort_pushdown = true`, `datafusion-common-54.0.0/src/config.rs:1289`; `try_pushdown_sort` hooks at `datafusion-datasource-54.0.0/src/file.rs:260`) and sort-aware machinery keying off declared orderings would produce exactly "identical displayed plans, different execution" | the config flag is a clean same-binary lever: repeat the ordering-toggle pairs with `datafusion.optimizer.enable_sort_pushdown = false` — if the effect vanishes, H5 is pinned | defer to row 38 with the path named; if pinned to `PushdownSort`, also evaluate implementing `try_pushdown_sort` in the provider as its own row (audit note §already-in-54) |
