...
**Prerequisites:** Plans 1–7, 10, 13, 14, 20, and 29 executed (all are — this is what the re-author verified: `cache.rs` with plan-20 counters, `metadata_cache.rs`, the plan-29 streaming writer). Independent of plan 32 (bench suites) — no overlapping code; if 32 is in flight, merge doc edits textually. After landing, a `bench hits queries` re-run vs the recorded baseline measures the prewarm effect on first-query latency (optional, manual).
