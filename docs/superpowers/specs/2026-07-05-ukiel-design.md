...
**Result formats.** `POST /api/query` returns results in one of three encodings (`docs/notes/2026-07-06-columnar-results.md`), selected by an optional `format` field (or the `Accept` header): `rows` (default; row-oriented JSON, now with an additive `schema` block), `columns` (columnar JSON — one value array per column, ~half the size on wide results), or `arrow` (Arrow IPC stream, lossless, zero re-parse for Arrow-native clients). Both JSON formats' `schema` block resolves each column's Ukiel type via an inert `ukiel:type` Arrow field-metadata hint (recovers `timestamp_ms`, which stores as `Int64`, and survives plain column references); computed columns fall back to the Arrow type. Errors are always JSON regardless of requested format. Formatting is strictly downstream of `df.collect()`, so isolation and `SQLOptions` are unaffected.
