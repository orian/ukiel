...
| S9 | **Compaction failover** *(plan 41)* | active-active compaction | A replica claims a partition and dies mid-merge without releasing anything, with the fleet cursor already at feed head. A peer does no work on that partition while the lease is live, then reclaims it after expiry and converges **with no new feed event** — only live state can reveal the abandoned backlog. |
| S10 | **Catalog outage** *(plan 42; `make e2e-ha`)* | role-local recovery | A managed-failover-shaped outage in front of PostgreSQL (Toxiproxy). Liveness never flinches, readiness turns 503, a new query is refused rather than answered from a stale plan, ingest stops advancing offsets, no role and no process exits — and afterwards the roles pass Degraded → Reconciling → Healthy on their own, with every event produced during the outage visible exactly once. |
| S11 | **Lost commit acknowledgement** *(plan 43; `make e2e-ha`)* | mutation decidability | The case S10 explicitly could not reach: the transaction is durable and the caller never learns it. A fault seam *inside* the commit path (feature-gated, absent from production builds) fires on either side of `COMMIT`; both sides hand the caller the identical transport error. Proves ingest and compaction converge exactly once under **both** answers, that a durable merge is reconciled *before* its expired lease is judged, and that a key reused under a different fingerprint fails permanently and mutates nothing. |

All scenarios S1–S11 are implemented in `crates/ukiel-e2e/tests/` (plus an S0 smoke); this table remains the authoritative statement of what each one must keep proving. S10 and S11 are behind the opt-in Toxiproxy compose profile (`make e2e-ha`) and skip themselves under a plain `make e2e`.

**On fault seams.** S11 is the one scenario that cannot be driven from outside the process. An outage seen from the network never tells you which side of `COMMIT` the connection died on — a proxy can cut the wire, but it cannot say "this transaction committed and the caller will never learn it", which is precisely the event that makes a mutation undecidable. So the seam lives in `ukiel-catalog` behind a `fault-injection` feature: instance-scoped (no globals), one-shot (a fault that fires every time models a broken database, not a lost acknowledgement), and compiled out of any build that does not ask for it. It is enabled from `ukiel-e2e`'s dev-dependencies, which the resolver keeps out of `cargo build -p ukield` — there is no environment variable and no TOML key, so it cannot become an operator-reachable knob.
